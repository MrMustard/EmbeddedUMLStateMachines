...
event_status_t protimer_state_machine(protimer_t *const mobj, event_t const *const e)
{
	...
	case STATE_IDLE:
	{
		return protimer_state_handler_IDLE(mobj, e);//execute handler of the state IDLE
	}

	case STATE_TIME_SET:
	{

		return protimer_state_handler_TIME_SET(mobj, e); //execute handler of the state TIMER SET
	}

	case STATE_COUNTDOWN:
	{
		return protimer_state_handler_COUNTDOWN(mobj, e);//execute handler of the state COUNTDOWN
	}
	case STATE_PAUSE:
	{

		return protimer_state_handler_PAUSE(mobj, e);////execute handler of the state PAUSE
	}

	case STATE_STAT:
	{

		return protimer_state_handler_STAT(mobj, e);////execute handler of the state STAT
	}
...
void protimer_event_dispatcher(protimer_t *const mobj, event_t *const e)
{
	...
	/*only when exits an event transition we have to do the next steps
	 * STEP 1: run the exit action for the source state
	 * STEP 2: run the entry action for the target state*/
